...
SELECT * FROM BOOK;
-- EMP와 유사한 EMP01 : EMPNO(숫자4), ENAME(문자-20), SAL(숫자7,2)
CREATE TABLE EMP01(
    EMPNO NUMBER(4),
    ENAME VARCHAR2(20),
    SAL   NUMBER(7,2)
...
ALTER TABLE EMP03 MODIFY(JOB VARCHAR2(5)); -- NULL이면 다 바꿔
ALTER TABLE EMP03 MODIFY(ENAME VARCHAR2(200));
ALTER TABLE EMP03 MODIFY(ENAME VARCHAR2(5));--6byte자료 있어 불가
--(3) 필드 삭제 (DROP)
SELECT * FROM EMP03;
...
UPDATE EMP01 SET (SAL, HIREDATE)=(SELECT SAL, HIREDATE FROM EMP01
                               WHERE ENAME='KING');
-- cf. UPDATE 테이블명 SET 필드명=값, 필드명=값...
...
CREATE TABLE EMP1(      -- 제약조건은 부적합한 데이터 삽입 방지 수단
    EMPNO NUMBER(4) PRIMARY KEY, -- 제약조건1. PRIMARY키
    ...
    COMM  NUMBER(7,2),
    DEPTNO NUMBER(2) REFERENCES DEPT1(DEPTNO) -- 제약조건6. FOREIGN키
);
SELECT * FROM EMP1;
