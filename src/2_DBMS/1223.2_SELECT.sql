...
SELECT * FROM EMP WHERE ENAME LIKE 'M%'; -- %:0글자이상 / _:한글자
...
-- 연봉이 2000이상인 직원의 이름, 연봉을 출력(연봉이 높은 순)
   -- ORDER BY 절에는 필드별명 사용가능. WHERE 절에는 필드별명 사용 불가
SELECT ENAME, SAL*12+NVL(COMM,0) 연봉 FROM EMP
...
-- 날짜형->문자형 TO_CHAR(HIREADTE,'YYYY/MM/DD')-> '1982/12/12'
...
--10.	EMP 테이블에서 job이 CLERK이거나 ANALYST이고,
    -- sal이 1000,3000,5000이 아닌 모든 사원의 정보를 출력
...
--11.	EMP 테이블에서 ename에 L이 두 자가 있고 
-- deptno가 30이거나 또는 mgr이 7782인 사원의 
...
--16.	사원 테이블에서연봉(SAL*12)이 35000 이상인 사번, 
--사원명, 연봉을 검색 하시오.
